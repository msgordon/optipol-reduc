...
Updated to python3 compatibility, September 2017 (M. Gordon)
"""
...
class cosmicsimage:
    def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0, sigclip = 5.0, sigfrac = 0.3, objlim = 5.0, satlevel = 50000.0, verbose=True):
        """
        sigclip : increase this if you detect cosmics where there are none. Default is 5, a good value for earth-bound images.
        objlim : increase this if normal stars are detected as cosmics. Default is 5, a good value for earth-bound images.
    ...
    def __str__(self):
        ...
        """
        stringlist = ["Input array : (%i, %i), %s" % (self.rawarray.shape[0], self.rawarray.shape[1], self.rawarray.dtype.name),
...
def fromfits(infilename, hdu = 0, verbose = True):
        ...
        if verbose :
                ...
                print("Internal array type : %s" % pixelarray.dtype.name)
...
def rebin(a, newshape):
    ...
    #print(factor)
    evList = ['a.reshape('] + \
             ['newshape[%i],factor[%i],'%(i,i) for i in range(lenShape)] + \
             [')'] + ['.sum(%i)'%(i+1) for i in range(lenShape)] + \
             ['/factor[%i]'%i for i in range(lenShape)]

    #print(''.join(evList));exit()
...
"""
    ...
    evList = ['a.reshape('] + \
             ['args[%i],factor[%i],'%(i,i) for i in range(lenShape)] + \
             [')'] + ['.sum(%i)'%(i+1) for i in range(lenShape)] + \
             ['/factor[%i]'%i for i in range(lenShape)]
    #print(''.join(evList));exit()
    return eval(''.join(evList))
...
def rebin2x2(a):
    ...
    return rebin(a, np.asarray(inshape/2,dtype=int))
